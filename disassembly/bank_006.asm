...
;; Dialog box drawing (S97 r2, PyBoy-measured). The field dialog machine
;; ($C915 state -> table $67B9) draws a 5x20-cell box at [$C919/$C91A] (screen
;; rows 13-17, or rows 0-4 when the player is low on screen): state 0 backs the
;; room's tiles up to $C100 + row*20; states 2/6 draw the middle rows through
;; LoadMapS_6939 ($FE, $E0 x18, $FF); state 9 draws the $FA/$EF/$FB top and
;; $FC/$EE/$FD bottom frames; states 12/15/19 put the tiles back through
;; LoadMapS_6b3d. Only TILE ids are written: on GBC the box keeps the room's
;; attributes and looks cream only because the engine forces colour 1 of every
;; BG palette to $6BFF. Text is typed into canvas tiles $B0-$C1 / $C2-$D3.
;; patches/: same-size far calls to bank $73 entries 14-16 (attrs -> 7 in
;; free-colour custom rooms).
LoadMapS_6939:
jr_006_6939:
    ld a, $fe
    call Write_gfx_tile
