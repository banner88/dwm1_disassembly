...
HramUnk11_406e:
    jp NewAttrHandler                ; HOOK: id-indexed new-species attr fork ($406e -> NewAttrHandler @ $792d)
    db $2d, $41, $85, $6f, $3e, $00, $8c, $67, $f0, $ca, $b6, $e0, $ca, $c9  ; dead tail of orig 17B body (byte-fidelity)
...
jr_011_4132:
    ...
    dec b
    inc b
    dw $4184                         ; layout level-1 slot for id 224 (sub $60 @ $413f); editor writes per new species
    ld b, $05
    ld [bc], a
    ld [bc], a
    nop
...
; Hooked from HramUnk11_406e (jp $792d) because id>=224's attr index overshoots the
; 87-entry table into live layout data ($418d=$41 -> stray Y-flip + green palette).
; For species>=224, supply a CLEAN attr from NewFollowerAttrTable[species-224]:
; clear the flip + palette bits ($98 mask) and OR the table byte. Else: original
; behavior ([$ca] |= [$412d + (species-$80)]). Lives at $792d (start of bank-$11
; trailing padding); 42B handler replaces 42 of 1747 $00 bytes (byte-neutral).
; =============================================================================
NewAttrHandler:
    ldh a, [$c7]                     ; A = adjusted index (species-$80)
    cp $60
    jr nc, .newspecies               ; species>=224 -> id-indexed attr table
    ld hl, $412d                     ; --- original: [$ca] |= [$412d + (species-$80)] ---
    add l
    ld l, a
    ld a, $00
    adc h
...
.newspecies:
    ...
    ldh [$ca], a
    ret
NewFollowerAttrTable:                ; one byte per new species (id 224..); editor appends
    db $02                           ; id 224: OBJ palette 2 (blue) for blue-dragon follower
    nop
    nop
    nop
    nop
