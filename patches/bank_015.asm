...
jr_015_461d:
    ...
    xor a
    call FillNBytesWithRegA
    ; CF3 (S60): NEW GAME commit point (title-menu entry 1 -> $15:$45FC).
    ; The displaced $C8EA x $1100 image zero-fill moves into bank $73 entry
    ; 7, which ALSO zeroes the 17 farm in-use flags at their SRAM home — a
    ; new game must not inherit the previous save's SRAM-resident farm.
    ; (The look-alike fill in the POWER-ON init above stays vanilla: it runs
    ; on every boot, before a possible Continue-load, and must never touch
    ; the farm.) Same-size 10-byte window.
    ld h, $73
    ld l, $07
    rst $10
    nop
...
jr_015_475b:
    pop de
    ; CF3 (S60): slot advance -> bank $73 entry 2, same-size 8-byte window.
    ; jr_015_4761 keeps its ADDRESS (window start +5): fake-decoded data
    ; above (~L990) has a jr whose displacement byte encodes it — the label
    ; is pinned onto the rst $10 boundary; nothing real jumps here.
    push bc
    push hl
    ld hl, $7302
