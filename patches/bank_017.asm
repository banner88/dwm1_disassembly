...
label17_41d0:
    ...
    add hl, hl
    add hl, hl
    call HighBattlePal           ; G2 fork: id>=224 -> custom battle palette; else HL=$62FD+species*8
    nop                          ; (5x nop keep the original 8-byte add-base footprint;
    nop                          ;  bank $17 carries raw embedded pointers -> no byte shift)
    nop
    nop
    nop
    ld a, [$c81f]
...
; Corrected from user's verified-working ROM (palette slot order matters)
CustomPaletteColors_6B:
    db $B1, $5A, $FF, $7F, $8C, $21, $00, $00  ; palette 0
    db $00, $7D, $FF, $6B, $42, $7F, $00, $00  ; palette 1
    db $3F, $02, $FF, $2B, $1B, $00, $00, $00  ; palette 2
    db $EC, $04, $FF, $6B, $3A, $02, $00, $00  ; palette 3
    db $9A, $02, $7F, $3F, $2F, $01, $00, $00  ; palette 4
    db $ED, $04, $FF, $6B, $15, $1A, $00, $00  ; palette 5
    db $E1, $7F, $FF, $7F, $01, $7C, $00, $00  ; palette 6
    db $00, $00, $00, $00, $00, $00, $00, $00  ; palette 7

; ---------------------------------------------------------------------------
; HighBattlePal: battle-palette resolver fork for NEW species (id >= 224). G2.
; label17_41d0 computes HL = MonsterBattlePalettes($62FD) + species*8. For id 224
; that is $69FD, which OVERSHOOTS the 216-entry table into PaletteColorData (the
; battle palette table has no real slot for new species, unlike the battle GFX
; table). So the add-base is forked here: this routine is entered with HL =
; species*8; if H >= $07 (species*8 >= $700  <=>  species >= 224) it returns HL =
; &HighBattlePal.pal (a custom 8-byte RGB555 palette); otherwise it reproduces
; the vanilla add-base (HL = $62FD + species*8). Lands in bank $17 filler tail
; ($6ce0+), so NO existing byte in this pointer-bearing bank moves.
;   .pal = id-224 (Gorbunok) battle palette, matching Battle_sp224 in bank $7e:
;   c0=$4d67 royal-blue body | c1=$6bff cream backdrop | c2=$7fff white belly |
;   c3=$0000 black outline. Generated by tools/bake_follower_overflow.py.
; Editor: per-species battle recolour = give that species a slot/branch here.
HighBattlePal:
    ld a, h
    cp $07                      ; species*8 >= $700  ->  new species (id >= 224)
    jr nc, .custom
    ld a, l                     ; vanilla: HL = $62FD + species*8
